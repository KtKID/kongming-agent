...
DEFAULT_PYTEST_TIMEOUT_SECONDS = 600
...
def run_pytest(repo: Path, tests: Sequence[str]) -> int:
    ...
    try:
        ...
        print(f"pre-push pytest timed out after {timeout_seconds} seconds", file=sys.stderr)
        return 124


def pytest_timeout_seconds(environ: Mapping[str, str] | None = None) -> int:
    """读取 pytest 超时时间，非法值回落到默认快速门禁时长。"""

    env = environ or os.environ
    raw_value = env.get("KONGMING_PRE_PUSH_TIMEOUT_SECONDS") or env.get("PRE_PUSH_TIMEOUT_SECONDS")
    if raw_value is None:
        return DEFAULT_PYTEST_TIMEOUT_SECONDS
    try:
