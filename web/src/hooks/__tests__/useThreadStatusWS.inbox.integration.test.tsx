...
} from "@/features/approval-inbox/types";
    ...
    act(() => {
      ws.serverPush(
        makeSnapshotFrame([
          ...
          makeItem({ requestId: "s2", blockedByRule: "danger" }),
...
describe("approval.inbox.resolve 帧发送", () => {
  it("onopen 后 store.resolve(...) → ws.send 收到字面对齐的 frame", () => {
    ...
    // 不传 opts → 不应带 message / rememberEntry 字段
    expect("message" in parsed).toBe(false);
    expect("rememberEntry" in parsed).toBe(false);
  });

  it("resolve 携带 message + rememberEntry 时字段透传", () => {
    renderHook(() => useThreadStatusWS());
    const ws = latestWs();
    ...
    expect(parsed.rememberEntry).toBe("Shell");
  });
});
