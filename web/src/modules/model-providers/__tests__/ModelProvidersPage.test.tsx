...
const apiMocks = vi.hoisted(() => ({
  ...
  getConnectedModelFamilies: vi.fn(),
  testProvider: vi.fn(),
...
describe("ModelProvidersPage", () => {
  beforeEach(() => {
    ...
    apiMocks.getConnectedModelFamilies.mockResolvedValue([]);
  });
  ...
  it("连接弹窗可以重新读取本地连接状态", async () => {
    const user = userEvent.setup();
    apiMocks.getProviderConnections
    ...
    const minimaxRow = screen.getByTestId("provider-row-minimax");
    expect(within(minimaxRow).getByText("已连接")).toBeInTheDocument();
    expect(within(minimaxRow).getByText("已从本地配置读取到连接。")).toBeInTheDocument();
    expect(apiMocks.getProviderConnections).toHaveBeenCalledTimes(2);
    expect(apiMocks.getConnectedModelFamilies).toHaveBeenCalled();
  });
