...
interface ModelProvidersState {
  ...
  reloadDialogConnection: () => Promise<void>;
...
async function fetchProviderItems(): Promise<{
  items: ProviderListItem[];
  ...
  return {
    ...
    connectionError:
      connectionsResult.status === "rejected"
        ? describeError(connectionsResult.reason)
        : null,
  };
}
...
export const useModelProvidersStore = create<ModelProvidersState>((set, get) => ({
  ...
  load: async () => {
    ...
    set({
      items: result.items,
      loadStatus: "loaded",
      loadError: result.connectionError,
    });
  },
  ...
  reloadDialogConnection: async () => {
    const providerId = get().dialog.providerId;
    ...
    const result = await fetchProviderItems();
    const refreshed = result.items.find((item) => item.providerId === providerId);
    ...
    if (result.connectionError) {
      set({
        ...
        dialog: {
          ...get().dialog,
          reloadStatus: "error",
          message: `重新读取失败：${result.connectionError}`,
        },
      });
      return;
    }
