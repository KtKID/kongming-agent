import type { ApprovalInboxResolveFrame } from "./types";

/**
 * Module-level sender 引用（**不进 zustand store**）。
 *
 * 为什么不放 store：
 * useThreadStatusWS 在 ws.onopen / onclose / cleanup 三处会高频调 setSender(real|null)。
 ...
 *
 * 使用约定：
 * - hook（useThreadStatusWS）调 setSender(real) / setSender(null)
 * - store.resolve 用 getSender() 取当前 sender 调用
 * - 测试用 resetSender() 清空（避免跨用例污染）
 */

let _sender: ((frame: ApprovalInboxResolveFrame) => void) | null = null;

export function setSender(
  s: ((frame: ApprovalInboxResolveFrame) => void) | null,
): void {
  _sender = s;
}

export function getSender(): ((frame: ApprovalInboxResolveFrame) => void) | null {
  return _sender;
}
