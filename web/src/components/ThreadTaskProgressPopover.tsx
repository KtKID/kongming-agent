...
export function ThreadTaskProgressPopover({
  ...
  useEffect(() => {
    ...
    if (lastThreadIdRef.current !== threadId) {
      ...
      if (snapshot && shouldAutoOpenInitialSnapshot(snapshot)) {
        setOpen(true);
      }
      return;
    }
  ...
  }, [progressSignature, setOpen, snapshot, threadId]);
      ...
      {actualOpen ? (
        <>
          ...
          <section
            ...
            className={cn(
              "fixed z-50 w-[min(22rem,calc(100vw-1.5rem))] rounded-lg border border-border/80 bg-popover/95 p-2 text-popover-foreground shadow-2xl backdrop-blur-xl",
              mobileMode ? "left-2 right-2 top-14" : "right-3 top-14",
              panelClassName,
            )}
          >
            <div className="flex min-h-8 items-center justify-between gap-2 px-1 pb-2">
              ...
              <div className="flex items-center gap-0.5">
                <Button
                  ...
                  disabled={disabled || isLoading || isHistoryLoading}
                  onClick={() => {
                    void refresh();
                    void refreshHistory();
                  }}
                >
                  <RefreshCw
                    className={cn(
                      "h-3.5 w-3.5",
                      (isLoading || isHistoryLoading) && "animate-spin",
                    )}
                  />
                </Button>
                <Button
