...
async def test_session_grant_resolve_then_next_request_immediate_allow() -> None:
    """generic-chat-session-grant 端到端：用户在 resolve 帧带 ``rememberScope='session'``
    → manager 触发 ``ApprovalRules.add_session_grant`` → 下次同 (cwd, tool) 真
    走 ``is_immediate=True`` immediate allow（不创建 pending）。

    本测覆盖 fix-report-20260520-generic-chat-session-grant 第二阶段：
    A. resolve 写 grant + manager.request 一阶段 classify 命中 → bypass pending
    B. grant 不串其他 tool（隔离）

    设计：用极短 timeout (5s) + race 控制让 resolve 在 timeout 前发生。
    """
    policy = _FakePolicy(
        next_decision=_FakeDecision(auto_eligible=False, blocked_by_rule=None, timeout_ms=10_000),
        enabled_cwds=set(),  # cwd 未启用自动通过，强制必须人审
    )
    manager = _make_manager_with_policy(policy=policy, default_timeout_ms=10_000)

    # 第一次 request：阻塞等用户决策；模拟用户点「本 session 都同意」
    async def user_resolve_with_session_grant() -> None:
        for _ in range(50):
            if manager.pending_count > 0:
                break
            await asyncio.sleep(0.005)
        req_id = next(iter(manager._pending.keys()))
        # rememberScope=session → manager.resolve 触发 add_session_grant
        ok = manager.resolve(req_id, {"allow": True, "rememberScope": "session"})
        assert ok

    resolve_task = asyncio.create_task(user_resolve_with_session_grant())
    d1 = await manager.request(
        channel="generic_chat",
        thread_id="thread-session-grant",
        cwd="/proj/grant",
        tool_name="Bash",
    ...
    assert d1.outcome == "approved"
    assert d1.metadata.get("remember_for_session") is True

    # grant 真写入 ApprovalRules._thread_overrides
    assert ("/proj/grant", "Bash") in manager._rules._thread_overrides["thread-session-grant"]

    # 第二次 request 同 thread + cwd + tool → immediate allow，不创建 pending
    d2 = await manager.request(
        channel="generic_chat",
        thread_id="thread-session-grant",
        cwd="/proj/grant",
        tool_name="Bash",
    ...
    assert d2.outcome == "approved"
    assert d2.metadata.get("source") == "rule_immediate"
    # 没经过 pending（即使 rules.classify 命中也走 is_immediate 分支直接返回）
    assert manager.pending_count == 0
    assert manager.auto_approve_task_count == 0

    # 第三次：不同 tool（Edit）→ grant 不命中，正常走 ask + timeout
    # 用极短 default_timeout 让本测快速收尾
    manager_short = _make_manager_with_policy(policy=policy, default_timeout_ms=100)
    # 把 grant 直接搬到新 manager 验证隔离
    manager_short._rules._thread_overrides.setdefault("thread-session-grant", set()).add(
        ("/proj/grant", "Bash")
    )
    d3 = await manager_short.request(
        channel="generic_chat",
        thread_id="thread-session-grant",
        cwd="/proj/grant",
        tool_name="Edit",  # 不同 tool，不命中 grant
        tool_input={"path": "/proj/grant/a"},
    )
    assert d3.outcome == "rejected"
    assert d3.metadata.get("source") == "manager_timeout"


async def test_cancel_by_thread_clears_session_grants() -> None:
    """``cancel_by_thread`` 时同步清掉该 thread 的所有 session grants（R10 防泄漏）。

    覆盖 fix-report-20260520-generic-chat-session-grant 生命周期：
    用户关 tab / cell evict → cancel_by_thread → grant 必须清空，
    避免 thread 关闭后 grant 残留造成内存泄漏。
    """
    policy = _FakePolicy(
        next_decision=_FakeDecision(auto_eligible=False, blocked_by_rule=None, timeout_ms=10_000),
        enabled_cwds=set(),
    ...
    # 直接写 grant（绕开 resolve 路径，专测 cancel_by_thread 清理）
    manager._rules.add_session_grant(
        channel="generic_chat",
        thread_id="thread-evict",
        cwd="/proj/foo",
        tool_name="Bash",
    )
    manager._rules.add_session_grant(
    ...
    assert len(manager._rules._thread_overrides["thread-evict"]) == 2

    # cancel_by_thread 触发清理（pending 为空 → cancel 0 条，但仍清 grant）
    cancelled = manager.cancel_by_thread("thread-evict", reason="cell_evict")
    assert cancelled == 0  # pending 是空

    # grant 被清空
    assert "thread-evict" not in manager._rules._thread_overrides
    # 其他 thread 的 grant 不受影响
    assert ("/proj/foo", "Bash") in manager._rules._thread_overrides["thread-other"]
