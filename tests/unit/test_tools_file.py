...
async def test_read_file_returns_content(tmp_path: Path) -> None:
    ...
    tool = ReadFileTool()
    result = await tool.execute({"path": str(target)}, _ctx())
...
async def test_read_file_missing_path_returns_error(tmp_path: Path) -> None:
    tool = ReadFileTool()
    result = await tool.execute({"path": str(tmp_path / "no.txt")}, _ctx())
...
async def test_read_file_respects_max_bytes(tmp_path: Path) -> None:
    ...
    tool = ReadFileTool()
    result = await tool.execute({"path": str(target), "max_bytes": 3}, _ctx())
    assert result.ok is True
    assert result.data is not None
...
async def test_list_dir_missing_path_returns_error(tmp_path: Path) -> None:
    tool = ListDirTool()
    result = await tool.execute({"path": str(tmp_path / "no")}, _ctx())
    assert result.ok is False
