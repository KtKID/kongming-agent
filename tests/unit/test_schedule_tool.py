...
# ---------------------------------------------------------------------------
# v0.3 cron-delivery：default_timezone + default_delivery_channel
# ---------------------------------------------------------------------------


class TestScheduleToolDefaults:
    """v0.3 真实端到端：LLM 创建任务时不带 timezone / delivery，schedule_tool
    必须用配置默认值填好，否则 dispatcher SKIPPED + cron 时区偏移。

    实战教训：M3+M4+M5 单测全过但端到端 web 实测失败 —— 因为这两个默认值
    没填，task.delivery=None → dispatcher SKIPPED；timezone='UTC' → 13:52
    ...
    async def test_create_uses_default_delivery_when_not_specified(self, tmp_path: Path) -> None:
        ...
        关键回归：v0.3 之前 task.delivery=None → dispatcher 直接 SKIPPED →
        整套投递链路在 LLM 创建任务路径下永远不通。
        """
        ...
        assert task.delivery is not None, (
            "task.delivery 必须被 schedule_tool 默认填上；否则 dispatcher SKIPPED"
        )
    ...
    async def test_create_default_delivery_channel_cli_variant(self, tmp_path: Path) -> None:
        ...
        store = Store(home_dir=tmp_path / "cron")
        tool = build_schedule_tool(store, default_delivery_channel="cli")
        r = await tool.execute(
            {
                "action": "create",
                "name": "cli 提醒",
    ...
    async def test_create_uses_default_timezone_when_not_specified(self, tmp_path: Path) -> None:
        ...
        创建 cron 任务，时间被错误地按 UTC 解释，结果偏 8 小时不触发。
        """
        store = Store(home_dir=tmp_path / "cron")
    ...
    async def test_create_explicit_timezone_overrides_default(self, tmp_path: Path) -> None:
        ...
        assert r.ok is True
        task = store.get_task(r.data["task_id"])
        assert task.trigger.timezone == "America/New_York"

    async def test_register_helper_threads_cfg_defaults(self, tmp_path: Path) -> None:
        """``register_schedule_tool_if_enabled`` 必须把
        ``cfg.scheduler.default_timezone`` / ``default_delivery_channel``
        透传给 schedule_tool。

        关键回归：装配链路漏一层透传 = 配置改了也不生效；用户最初痛点
        "时区在 settings 里有配置的" 就是这个 contract。
        """
        ...
        cfg = Config(
            model=ModelConfig(name="m", base_url="http://127.0.0.1:1234/v1", api_key=""),
            scheduler=SchedulerConfig(
                enabled=True,
                home=tmp_path / "cron",
                default_timezone="Asia/Shanghai",
                default_delivery_channel="cli",
            ),
        )
        registry: ToolRegistry = build_default_registry()
        store = register_schedule_tool_if_enabled(registry, cfg)
        assert store is not None

        tool = registry["schedule"]
        # 直接通过 tool 创建任务，验证 cfg 默认透传到 schedule_tool 实例
        r = await tool.execute(
            {
                "action": "create",
                "name": "透传测试",
