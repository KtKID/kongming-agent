...
def _expected_at_line(path: Path) -> str:
    path_str = str(path)
    needs_quote = (
...
class TestBuildSingle:
    ...
    def test_single_png_atfile_prefix(self, tmp_path: Path) -> None:
        ...
        assert result == f"{_expected_at_line(path)}\n\n"
...
class TestBuildMultiple:
    ...
    def test_two_attachments_keep_order(self, tmp_path: Path) -> None:
        ...
        # 期望两行 + 末尾空行
        assert result == f"{_expected_at_line(path_a)}\n{_expected_at_line(path_b)}\n\n"
    ...
    def test_three_attachments(self, tmp_path: Path) -> None:
        ...
        for path in expected_paths:
            assert _expected_at_line(path) in result
        # 顺序
        idx0 = result.index(_expected_at_line(expected_paths[0]))
        idx1 = result.index(_expected_at_line(expected_paths[1]))
        idx2 = result.index(_expected_at_line(expected_paths[2]))
...
class TestPathWithSpecialChars:
    ...
    def test_plain_unix_path_uses_regular(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        class FakeUnixPath:
            def __str__(self) -> str:
                return f"/tmp/kongming/{thread_id}/{asset_id}.png"

            def is_file(self) -> bool:
                return True

        fake_unix_path = FakeUnixPath()
        monkeypatch.setattr(storage, "asset_path", lambda **_: fake_unix_path)
        builder = AttachmentPrefixBuilder(storage)
...
class TestFallbackSkip:
    ...
    def test_partial_failure_keeps_valid(
        ...
        assert result == f"{_expected_at_line(good_path)}\n\n"
