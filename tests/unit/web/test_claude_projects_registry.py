"""Pytest for ``src/web/claude_code/projects_registry.py``.
...
class TestLoadRegistry:
    ...
    def test_corrupted_json_returns_empty_with_warning(
        ...
        with caplog.at_level(logging.WARNING, logger="web.claude_code.projects_registry"):
...
class TestAddProject:
    ...
    def test_atomic_write_no_tmp_residue(self, tmp_path: Path) -> None:
        ...
        path = claude_projects_path(tmp_path)
        # 实现用 path.with_suffix(".json.tmp") → claude_projects.json.tmp
...
class TestBootstrap:
    def test_creates_registry_when_file_missing(self, tmp_path: Path) -> None:
        assert not claude_projects_path(tmp_path).exists()
...
class TestMigrate:
    ...
    def test_migrate_only_claude_code_with_nonempty_cwd(self, tmp_path: Path) -> None:
        """过滤 backend_kind != 'claude_code' 的、cwd 空的。"""
        # claude_code + cwd 非空 → 应迁移
        write_thread_metadata(
            tmp_path,
            _make_meta(thread_id="thread-aaaaaaaaaaaa", cwd="/proj/a"),
        )
        # claude_code + cwd 空 → 跳过
        write_thread_metadata(
            tmp_path,
            _make_meta(thread_id="thread-bbbbbbbbbbbb", cwd=""),
        )
        # codex 后端 → 跳过（即使 cwd 非空）
        write_thread_metadata(
            tmp_path,
            _make_meta(
        ...
        # 又一条 claude_code + cwd 非空（不同 cwd）→ 应迁移
        write_thread_metadata(
            tmp_path,
