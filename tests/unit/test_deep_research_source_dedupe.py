...
def test_canonicalize_url_tolerates_malformed_port() -> None:
    """验证非法端口降级，输入为坏端口 URL，输出为可追踪 canonical URL。"""
    result = canonicalize_url("https://example.com:bad/path?utm_source=news")

    assert result == "https://example.com/path"


def test_canonicalize_url_tolerates_malformed_bracket_url() -> None:
    """验证非法 bracket URL 降级，输入为坏 IPv6 形态，输出为可追踪 canonical URL。"""
    result = canonicalize_url("https://[::1/path?utm_source=news#frag")
