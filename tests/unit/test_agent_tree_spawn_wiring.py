...
2. **#5 spawn 工具入口**：SpawnSubAgentTool 调 AgentManager.spawn（duck-typed handle），
...
def _make_approval_manager() -> ApprovalManager:
    return ApprovalManager(rules=ApprovalRules(), event_sinks=[])


async def test_pending_approval_carries_agent_id() -> None:
    """_PendingApproval.agent_id 字段存在；request() 透传 agent_id 到 pending。"""
    manager = _make_approval_manager()
...
async def test_cancel_by_agent_only_matches_target_agent() -> None:
    ...
    def spawn(
        ...
        *,
        ...
        self.spawn_calls.append(
            {
                "parent": parent_cell.agent_id,
                "spec_name": spec.name,
                "seed": seed_message.content,
                "cwd": cwd,
            }
        )
...
async def test_spawn_subagent_tool_dispatches_via_handle() -> None:
    """SpawnSubAgentTool 调 manager.spawn，返回 {child_id, status:dispatched}。"""
    fake_manager = _RecordingAgentManager()
    handle = AgentTreeSpawnHandle()
    handle.bind(fake_manager)
    tool = build_spawn_subagent_tool(handle, parent_model="m")
    ...
    assert fake_manager.spawn_calls[0]["seed"] == "summarize the report"
    assert fake_manager.spawn_calls[0]["cwd"] == "/tmp/work"
...
async def test_spawn_subagent_tool_raises_when_handle_unbound() -> None:
    """未绑定 AgentManager → RuntimeError（spawn_subagent 需要 agent-tree）。"""
    handle = AgentTreeSpawnHandle()
    tool = build_spawn_subagent_tool(handle)
