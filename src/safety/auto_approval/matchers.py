...
# Shell 工具名：Claude SDK 用 ``Bash``，kongming native tools 用 ``run_shell``。
_BASH_TOOL_NAMES: frozenset[str] = frozenset(["Bash", "run_shell"])

# Edit 系列工具：同时覆盖 Claude SDK 与 kongming native tools。
_EDIT_TOOL_NAMES: frozenset[str] = frozenset(
    ["Edit", "Write", "NotebookEdit", "MultiEdit", "write_file", "edit_file"],
)
