...
class TaskRecord:
    ...
    task_id: str  # 唯一标识（uuid4().hex[:12]）
    agent_id: str  # 归属 agent
    parent_task_id: str | None  # 父 task（spawn 链）；根节点 = None
    kind: TaskKind  # 只 2 种（需求要点 12，无 detach）
    ...
    started_at: int  # 审计保留（迁移自 SubAgentLifecycleRecord；用 now_epoch_ms，epoch 毫秒）
...
class TaskRegistry:
    ...
    async def cancel_subtree(self, agent_id: str) -> None:
        ...
        关键链路：入口即 close_registry（置关门标志，防新登记漏杀）→
        收集该 agent 子树所有 TaskRecord → 按 kind 分组 →
        ...
        """
        # 入口即置关门标志，防「dispatched 与登记之间被打断」漏杀孤儿子。
        self._closed = True
        ...
        try:
            ...
            if record.status in {"cancelled", "failed", "completed"}:
                if record.finished_at is None:
                    record.finished_at = now_epoch_ms()
                return
            if task.cancelled():
