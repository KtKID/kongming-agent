...
- 与 :mod:`web.integrations.codex.normalizer`（live 流翻译器）**独立实现**——live
  ...
  不如直接翻译更清晰
- 模块只 import 标准库 + :mod:`web.integrations.codex.normalizer` 的 strip 工具；
...
from hosts.web.integrations.codex.normalizer import _PAIRED_BLOCK_RE, INTERNAL_CONTENT_PREFIXES

if TYPE_CHECKING:
    from hosts.web.app_support.llm_protocol import NormalizedMessage  # noqa: F401
...
def _normalize_usage(usage: Any) -> dict[str, Any]:
    ...
    字段映射与 :func:`web.integrations.codex.normalizer._normalize_usage` 一致。
    """
