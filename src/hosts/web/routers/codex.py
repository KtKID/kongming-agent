...
端点（与 ``src/hosts/web/routers/claude.py`` 完全对称，仅模块路径与字段差异）。

与 ``src/hosts/web/integrations/codex/route.py``（WebSocket 端点）平级；
...
from hosts.web.integrations.codex.projects_scanner import CodexProjectSummary, list_codex_projects
from hosts.web.protocol.rest_models import AddProjectRequest, ProjectRegistryEntryDTO
