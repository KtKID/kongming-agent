...
class WorkflowRunViewerManager:
    ...
    def _report_summaries(self, bundle: WorkflowArtifactBundle) -> list[SubAgentReportSummaryDTO]:
        ...
        for report in bundle.reports:
            ...
            if record is not None:
                ...
                activity_events, activity_diagnostics = _project_subagent_activity(bundle, record)
                diagnostics.extend(activity_diagnostics)
                snapshot = dict(record.subagent_json)
            else:
                activity_events = []
                snapshot = {}
            summaries.append(
    ...
    def _read_subagents(
        ...
        reports_by_task_run = {_task_run_id_from_report(report): report for report in reports}
        ...
        for subagent_path in sorted(agents_dir.glob("*/subagent.json"), key=lambda p: str(p)):
            ...
            records.append(
                SubAgentRecordRef(
                    ...
                    completed_run_id=_str_or_none(payload.get("completed_run_id"))
                    or _str_or_none(reports_by_task_run.get(task_run_id, {}).get("run_id")),
                    ...
                    report_path=_path_or_none(
                        reports_by_task_run.get(task_run_id, {}).get("report_path")
                    ),
...
def _project_subagent_activity(
    ...
    record: SubAgentRecordRef,
) -> tuple[list[SubAgentActivityEventDTO], list[WorkflowDiagnosticDTO]]:
    diagnostics: list[WorkflowDiagnosticDTO] = []
    events: list[tuple[str | None, int, SubAgentActivityEventDTO]] = []
    task_id_counts: dict[str, int] = {}
    for item in bundle.subagent_records:
        if item.task_id:
            task_id_counts[item.task_id] = task_id_counts.get(item.task_id, 0) + 1

    for index, event in enumerate(bundle.audit_events):
        payload_raw = event.get("payload")
        payload = payload_raw if isinstance(payload_raw, dict) else {}
        match_source = _subagent_activity_match_source(
            payload=payload,
            record=record,
            workflow_dir=bundle.workflow_dir,
            task_id_counts=task_id_counts,
        )
        if match_source is None:
            continue
        action = _str_or_none(event.get("action")) or "unknown"
        ts = _str_or_none(event.get("ts"))
        if match_source != "task_run_id":
            diagnostics.append(
                WorkflowDiagnosticDTO(
                    code="subagent_activity.fallback_match",
                    severity="info",
                    message=f"{action} 通过 {match_source} 匹配到 {record.task_run_id}",
                    path="audit.jsonl",
                )
            )
        events.append(
            (
                ...
                index,
                SubAgentActivityEventDTO(
                    activity_id=f"audit-{index}",
                    activity_type=_activity_type(action),
                    ts=ts,
                    title=_activity_title(action),
                    task_run_id=_str_or_none(payload.get("task_run_id")) or record.task_run_id,
                    task_id=_str_or_none(payload.get("task_id")) or record.task_id,
                    session_id=_str_or_none(payload.get("session_id")) or record.child_session_id,
                    run_id=_str_or_none(payload.get("run_id")) or record.completed_run_id,
                    status=_activity_status(payload),
                    summary=_activity_summary(action, payload),
                    source="audit.jsonl",
    ...
    events.sort(key=lambda item: (item[0] is None, item[0] or "", item[1]))
    return [event for _, _, event in events], diagnostics


def _subagent_activity_match_source(
    *,
    ...
    if _str_or_none(payload.get("task_run_id")) == record.task_run_id:
        return "task_run_id"
    if (
        record.child_session_id
        and _str_or_none(payload.get("session_id")) == record.child_session_id
    ):
        return "session_id"
    if record.completed_run_id and _str_or_none(payload.get("run_id")) == record.completed_run_id:
        return "run_id"
    task_id = _str_or_none(payload.get("task_id"))
    if record.task_id and task_id == record.task_id and task_id_counts.get(task_id) == 1:
        return "task_id"
    if _payload_path_matches_task_run(
        payload.get("subagent_json_path"),
        workflow_dir=workflow_dir,
        task_run_id=record.task_run_id,
        expected_suffix=("agents", record.task_run_id, "subagent.json"),
    ):
        return "subagent_json_path"
    if _payload_path_matches_task_run(
        payload.get("report_path"),
        workflow_dir=workflow_dir,
        task_run_id=record.task_run_id,
        expected_suffix=("reports", f"{record.task_run_id}.json"),
    ):
        return "report_path"
    return None


def _payload_path_matches_task_run(
    value: Any,
    *,
    workflow_dir: Path,
    task_run_id: str,
    ...
    try:
        rel_parts = path.resolve().relative_to(workflow_dir.resolve()).parts
    except ValueError:
        rel_parts = path.parts
    normalized = tuple(part.replace("\\", "/") for part in rel_parts)
    return normalized[-len(expected_suffix) :] == expected_suffix or path.stem == task_run_id


def _activity_type(action: str) -> str:
    mapping = {
        "subagent_created": "created",
        "subagent_grant_bound": "grant_bound",
        "subagent_approval_decided": "approval_decided",
        "agent_completed": "completed",
        "agent_failed": "failed",
        "subagent_reported": "reported",
    }
    return mapping.get(action, action.replace("subagent_", "").replace("agent_", ""))


def _activity_title(action: str) -> str:
    mapping = {
        "subagent_created": "子 agent 已创建",
        "subagent_grant_bound": "授权已绑定",
        "subagent_approval_decided": "审批已决策",
        "agent_completed": "子 agent 已完成",
        "agent_failed": "子 agent 失败",
        "subagent_reported": "报告已写入",
    }
    return mapping.get(action, action.replace("_", " "))


def _activity_status(payload: dict[str, Any]) -> str | None:
    for key in ("status", "decision", "approval_status"):
...
def _activity_summary(action: str, payload: dict[str, Any]) -> str | None:
    if action == "subagent_created":
        return _join_summary_parts(
            _str_or_none(payload.get("task_name")),
            _str_or_none(payload.get("working_dir")),
        )
    if action == "subagent_grant_bound":
        return _join_summary_parts(
            _str_or_none(payload.get("mode")),
            _str_or_none(payload.get("working_dir")),
        )
    if action == "subagent_approval_decided":
        return _join_summary_parts(
            _activity_status(payload),
            _str_or_none(payload.get("tool_name")),
        )
    if action in {"agent_completed", "agent_failed"}:
        return _join_summary_parts(
            _activity_status(payload),
            _str_or_none(payload.get("error_message")),
        )
    if action == "subagent_reported":
        return _join_summary_parts(
            _str_or_none(payload.get("report_path")),
            _str_or_none(payload.get("content_digest")),
        )
    return _join_summary_parts(_str_or_none(payload.get("summary")), _activity_status(payload))


def _join_summary_parts(*parts: str | None) -> str | None:
    compact = [" ".join(part.split()) for part in parts if part and part.strip()]
