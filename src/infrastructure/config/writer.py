...
def round_trip_update(
    ...
    *,
    create_missing_paths: set[str] | None = None,
    ...
    # Windows 上持有原文件 fd 会让 ``Path.replace`` 更容易撞到 sharing violation。
    # sidecar lock file 保持同目录互斥语义，同时不影响对 yaml_path 的原子替换。
    ...
    try:
        ...
        # 4) 按 path 设值（嵌套 dict）
        creatable_paths = create_missing_paths or set()
        for item in patch_list:
            _set_by_dot_path(
                doc,
                item.path,
                item.value,
                create_missing=item.path in creatable_paths,
            )
...
def _lock_path_for(yaml_path: Path) -> Path:
    """同目录 sidecar lock file：``.<name>.lock``。"""
    return yaml_path.with_name(f".{yaml_path.name}{_LOCK_SUFFIX}")


def _lock_fd(fd: int) -> None:
    """平台独占锁：Windows 用 ``msvcrt``，POSIX 用 ``fcntl``。"""
...
def _prime_lock_file(fd: int) -> None:
    """Windows ``msvcrt.locking`` 需要锁一个已存在的字节区间。"""
...
def _set_by_dot_path(
    ...
    *,
    ...
    if last_key not in cursor and not create_missing:
