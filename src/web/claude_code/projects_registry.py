...
- 维护用户在 Web UI 上"加过的 claude 项目工作目录"列表，与
...
- 历史数据迁移走 :func:`migrate_from_thread_metadata`，扫已有 thread
  的 ``backend_kind == "claude_code"`` distinct cwd 一次性入库。

公共 API：

- :class:`ProjectRegistryEntry` —— 单条登记项 dataclass（frozen）
- :func:`claude_projects_path` —— 路径常量
...
def claude_projects_path(home: Path) -> Path:
    """返回 registry 文件路径 ``<home>/web/claude_projects.json``。
    ...
    """
    return home / "web" / "claude_projects.json"
...
def load_registry(home: Path) -> list[ProjectRegistryEntry]:
    ...
    """
    path = claude_projects_path(home)
    if not path.is_file():
        return []
    try:
        raw = path.read_text(encoding="utf-8")
    except OSError as exc:
        logger.warning("failed to read claude projects registry at %s: %s", path, exc)
        return []
    try:
        data = json.loads(raw)
    except (ValueError, json.JSONDecodeError) as exc:
        logger.warning("claude projects registry JSON corrupted at %s: %s", path, exc)
        return []
    if not isinstance(data, dict):
        logger.warning("claude projects registry not a JSON object at %s", path)
        return []
    version_raw = data.get("version", CURRENT_VERSION)
    if not isinstance(version_raw, int):
        logger.warning(
            "claude projects registry version not int at %s: %r",
            path,
            version_raw,
        )
        return []
    if version_raw > CURRENT_VERSION:
        logger.warning(
            "claude projects registry version %s exceeds known %s at %s; treating as empty",
            version_raw,
            CURRENT_VERSION,
            path,
        )
    ...
    if not isinstance(projects_raw, list):
        logger.warning(
            "claude projects registry has no projects list at %s: %r",
            path,
            projects_raw,
        )
        return []
...
def _write_registry(home: Path, entries: list[ProjectRegistryEntry]) -> None:
    ...
    实现：先写 ``claude_projects.json.tmp``，再 ``os.replace`` 到目标
...
def migrate_from_thread_metadata(home: Path) -> int:
    """从 thread metadata 反向迁移 claude_code distinct cwd 到 registry。

    扫 ``<home>/web/threads/*/metadata.json``，对所有
    ``backend_kind == "claude_code"`` 且 ``cwd != ""`` 的记录取 distinct cwd，
    依次 :func:`add_project`。
